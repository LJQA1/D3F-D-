...
#             nn.Sequential(Conv(c2, c2,5,g=c2,d=2),Conv(c2, c2,3,g=c2,d=1)) for x in ch)
#         self.cv3_ = nn.ModuleList(
#             nn.Sequential(Conv(c3, c3, 5,g=c3,d=2), Conv(c3, c3,3,g=c3,d=1)) for x in ch)
...
#         # else:
#         #     self.cv2 = nn.ModuleList(
#         #         nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch[1:] # ch[:-1]
#         #     )
#         #     self.cv3 = nn.ModuleList(
#         #         nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch[1:])
...
#             x2_ = self.cv2[i][0](x[i])
#             x3_ = self.cv3[i][0](x[i])
#             x[i] = torch.cat((self.cv2[i][1:](self.cv2_[i][0](x2_)+self.cv2_[i][1](x2_)),
#                               self.cv3[i][1:](self.cv3_[i][0](x3_)+self.cv3_[i][1](x3_))), 1)
#         # for i in range(self.nl):
...
#         #         # x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i)), 1)
#         #         x[i+1] = torch.cat((self.cv2[i](x[i+1]), self.cv3[i](x[i+1])), 1)
