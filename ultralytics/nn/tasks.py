...
from ultralytics.change_model.ConvMod import A2C2f_ConvMod,C3k2_ConvMod
from ultralytics.change_model.SPDConv import SPDConv

from ultralytics.change_model.MSLA import A2C2f_MSLA,MSLA
from ultralytics.change_model.focal_modulation_module import C3k2_FocalModulation
from ultralytics.change_model.ASSA import WindowAttention_ASSA,A2C2f_ASSA
from ultralytics.change_model.APConv import C3k2_PConv
from ultralytics.change_model.CAFMAttention import CAFMAttention
from ultralytics.change_model.Converse2D import Converse2D
from ultralytics.change_model.GRSA import A2C2f_GRSA
from ultralytics.change_model.CSA_ConvBlock import A2C2f_CSA_ConvBlock,C3k2_CSA_ConvBlock
from ultralytics.change_model.MWA import MWA_Module
from ultralytics.change_model.ESSamp import ESSamp
from ultralytics.change_model.Original import Original

from ultralytics.change_model.Diffclip import A2C2f_DiffAttention
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB, RepHDW):
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is AVG:
            c2 = ch[f]
            ...
            c1,c2  = args[0],args[1]
        elif m is Original:  # 注意用 is 而非 in（单模块）
            c1 = ch[f]  # 输入通道来自上一层
            c2 = args[0]  # 输出通道是 args 第一个参数
            k = args[1] if len(args) > 1 else 3  # 卷积核大小，默认3
            args = [c1 , c2 , k]  # 重新组织参数，传给模块 __init__
