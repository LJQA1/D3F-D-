...
class BaseTrainer:
    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            if RANK in {-1, 0}:  # 仅在主进程打印
                LOGGER.info(f"Epoch {epoch + 1}/{self.epochs} - Learning Rates: {self.lr}")
            ...
            if RANK in {-1, 0}:
                ...
                # 学习率输出设置
                loss_length = self.tloss.shape[0] if len(self.tloss.shape) else 1
                lr_info = f"LR: {self.optimizer.param_groups[0]['lr']:.6f}"  # 显示第一个 param_group 的学习率
                pbar.set_description(
                    ("%11s" * 2 + "%11.4g" * (2 + loss_length) + "%15s")
                    % (
                        f"{epoch + 1}/{self.epochs}",
                        f"{self._get_memory():.3g}G",  # GPU 内存
                        *(self.tloss if loss_length > 1 else torch.unsqueeze(self.tloss, 0)),  # losses
                        batch["cls"].shape[0],  # batch size
                        batch["img"].shape[-1],  # imgsz
                        lr_info,  # 新增：学习率显示
                    )
                )
