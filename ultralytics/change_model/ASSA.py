...
class WindowAttention_ASSA(nn.Module):
    def __init__(self, dim, win_size=None, num_heads=4, token_projection='linear', qkv_bias=True, qk_scale=None,
                 attn_drop=0.,
                 proj_drop=0.):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        # 确定目标边长，取 H 和 W 中的较大值
        target_size = max(original_H, original_W)
        ...
        # 由于 win_size 不固定，这里去掉相对位置偏置相关代码
        # 若有需要，可根据 target_size 动态计算相对位置偏置

        if mask is not None:
            # 这里假设 mask 处理逻辑不受尺寸变化影响，若有问题需调整
            nW = mask.shape[0]
            ratio = attn.size(-1)
        ...
        x = x.reshape(x.shape[0], target_size, target_size, x.shape[2]).permute(0, 3, 1, 2)
...
class ABlock_ASSA(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=1.2, area=1):
        ...
        self.attn = WindowAttention_ASSA(dim)
...
class A2C2f_ASSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock_ASSA(c_, c_ // 32, mlp_ratio, area) for _ in range(2)))
            if a2
...
if __name__ == "__main__":
    # 定义输入张量
    x = torch.randn(2, 64, 27, 32)

    # 实例化 WindowAttention_ASSA 模块，不提供 win_size
    attn_module = WindowAttention_ASSA(dim=x.size(1), num_heads=4)
