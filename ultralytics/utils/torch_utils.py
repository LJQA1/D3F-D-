...
def get_flops(model, imgsz=640):
    """Return a model's FLOPs (with support for custom modules like GELU and PixelUnshuffle)."""
    try:
        import torch.nn as nn
        ...
        def count_gelu(m, x, y):
            x = x[0]
            m.total_ops += 2 * x.numel()  # GELU ≈ 2 ops per element

        def count_pixel_unshuffle(m, x, y):
            pass  # no FLOPs

        CUSTOM_OPS = {
            nn.GELU: count_gelu,
            nn.PixelUnshuffle: count_pixel_unshuffle,
        }
        ...
        dummy_input = torch.zeros(
            (1, model.yaml.get('ch', 3), *imgsz),
            device=next(model.parameters()).device
        )

        # === 执行 profile ===
        flops, _ = profile(
            deepcopy(model.eval()),  # 防止影响训练状态
            inputs=(dummy_input,),
            custom_ops=CUSTOM_OPS,
            verbose=False
        )
        return flops / 1e9 * 2  # 转为 GFLOPs
