...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, WIoU=False, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU or WIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            from ultralytics.utils.wiou import IoU_Cal
...
#     h2 = b2_y2 - b2_y1 + eps
#
#     b1_center = torch.cat(((b1_x1 + w1 / 2), (b1_y1 + h1 / 2)), dim=-1)
#     b2_center = torch.cat(((b2_x1 + w2 / 2), (b2_y1 + h2 / 2)), dim=-1)
#
#     center_distance = ((b1_center - b2_center) ** 2).sum(dim=-1, keepdim=True) + eps
#     wh_distance = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4
#
#     wasserstein_2 = center_distance + wh_distance
#     similarity = torch.exp(-torch.sqrt(wasserstein_2) / constant)
#
#     return 1.0 - similarity  # 作为损失

def wasserstein_loss(pred, target, constant=5.0, scale=4.0, eps=1e-7):
    """
    Normalized Wasserstein Distance (NWD) for bounding boxes.
    Args:
        pred: predicted bounding boxes (tensor) [..., 4] (x1,y1,x2,y2)
        target: target bounding boxes (tensor) [..., 4] (x1,y1,x2,y2)
        constant: scaling factor (VisDrone使用5.0)
        scale: 输出缩放因子 (VisDrone使用4.0)
    Returns:
        nwd_loss: scale * (1 - NWD similarity)
    """
    # 解包坐标
    b1_x1, b1_y1, b1_x2, b1_y2 = pred.chunk(4, -1)
    b2_x1, b2_y1, b2_x2, b2_y2 = target.chunk(4, -1)
    ...
    # 中心点距离（归一化到[0,1]）
    center_x1 = (b1_x1 + w1 / 2)
    center_y1 = (b1_y1 + h1 / 2)
    center_x2 = (b2_x1 + w2 / 2)
    center_y2 = (b2_y1 + h2 / 2)
    center_distance = ((center_x1 - center_x2) ** 2 + (center_y1 - center_y2) ** 2) / 4

    # 宽高距离（归一化到[0,1]）
    wh_distance = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4

    # 综合距离并计算相似度（添加安全机制）
    wasserstein_distance = torch.clamp(center_distance + wh_distance, min=0, max=10)
    similarity = torch.exp(-wasserstein_distance / (constant + eps))

    return scale * (1 - similarity)